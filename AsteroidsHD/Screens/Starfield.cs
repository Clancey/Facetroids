...
namespace AsteroidsHD
{
    ...
    public class Starfield : IDisposable
    {
        ...
        public void LoadContent()
        {
			...
			//starTexture =  contentManager.Load<Texture2D>("Textures/whitepixel");
            starTexture = new Texture2D(graphicsDevice, 1, 1, false, SurfaceFormat.Color);
           // starTexture.SetData<Color>(new Color[] { Color.Red });
        ...
        public void Reset(Vector2 position)
        {
        	 ...
        	 int viewportWidth = screenManager.Width;
        	int viewportHeight = screenManager.Height;
            // recreate the stars
        ...
        public void Draw(Vector2 position)
        {
           ...
           // spriteBatch.Begin();
			...
			spriteBatch.Begin (SpriteSortMode.Deferred,  BlendState.NonPremultiplied, null, null, null, null, _view);
            for (int i = 0; i < stars.Length; i++)
            {
                ...
                if (stars[i].Y > 
                    (starfieldRectangle.Y + screenManager.Height))
                {
                    stars[i].X = starfieldRectangle.X + 
                        RandomMath.Random.Next(starfieldRectangle.Width);
                    stars[i].Y = starfieldRectangle.Y;
